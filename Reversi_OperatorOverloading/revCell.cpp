...
ostream & operator<<(ostream & outputStream, const Reversi & print) {
  ...
  for (int i = 0; i < print.getHeight(); ++i) {
    ...
    for (int j = 0; j < print.getWidth(); ++j) {
      //cout<<"h:"<<h<<"w:"<<w<<endl;
      if (print.gameCell[h][w].getColumn() == j && print.gameCell[h][w].getRow() == i) {
        outputStream << print.gameCell[h][w].getCharWhose();
        if (w + 1 < print.gameCell[h].size())
          w++;
        else {
          h++;
          w = 0;
          if (h == print.gameCell.size())
            h = 0;
        }
      } else
        outputStream << '.';
...
//This function take parameter as string , and return Cell 
// Cell -> row and column (as integer value)

Cell Reversi::operator[](string revParametre) {
  int t = 0;
  int j = 0;
  int length = revParametre.size();
  string temp1;
  string temp2;
  //it founds integer in string
  for (int i = 0; i < length; ++i) {
    if (isdigit(revParametre[i]) == 1) {
      temp1.push_back(revParametre[i]);
    }
    else {
      temp2.push_back(revParametre[i]);
    }
  }
  //values of new temporary string change as integer
  int rowCell = (atoi(temp1.c_str()));
  int columnCell = (temp2.length() - 1)*26 + int(temp2[0] - 'a');
  for (int i = 0; i < gameCell.size(); ++i) {
    for (int k = 0; k < gameCell[i].size(); ++k) {
      if (rowCell == (*this).gameCell[i][k].getRow() &&
              columnCell == (*this).gameCell[i][k].getColumn())
        return Cell(rowCell, columnCell, 'X');
...
//it takes parameter,and these parameter locate in cell but in gameCell's vector

Cell Reversi::operator()(string columnChar, int rowCount) {
  string temp2;
  int length = columnChar.size();
  for (int i = 0; i < length; ++i) {
    temp2.push_back(columnChar[i]);
  }
  int columnCell = (temp2.length() - 1)*26 + int(temp2[0] - 'a');
  for (int i = 0; i < gameCell.size(); ++i) {
    for (int k = 0; k < gameCell[i].size(); ++k) {
      if (rowCount == (*this).gameCell[i][k].getRow() &&
              columnCell == (*this).gameCell[i][k].getColumn())
        return Cell(rowCount, columnCell, 'X');
...
//it control for new user attack

Reversi Reversi::operator+=(const Cell & game) {
  //respectively it calls truth control and modify functions
  int rowY = game.getRow();
  int columnX = game.getColumn();
  char symbolChar = game.getCharWhose();
  if ((*this).truth_control(rowY, columnX, symbolChar) == true)
    (*this).modify_new(rowY, columnX, symbolChar);
  return (*this);
}
